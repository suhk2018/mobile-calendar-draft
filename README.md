...
- 달력을 위로 밀면 축소 달력과 일정 패널을 표시하고, 뒤로가기로 원래 달력에 복귀
...
- 하단 `캘린더 / 지도 / 메모 / 목록` 탭으로 화면을 전환하고 한 손으로 캘린더 목록 열기
...
- 기념일 화면에는 가장 가까운 기념일과 생일을 날짜순으로 최대 5개 표시
...
- 휴대폰 뒤로가기로 목록, 지도 확대, 장소·일정 입력, 날짜 일정 화면을 차례로 닫고 첫 달력에서 한 번 더 누르면 앱 종료
